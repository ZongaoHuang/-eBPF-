...
struct agent_config_data
{
  ...
  static agent_config_data from_toml_file(const std::string &file_path);
  /// parse config from json files
  static agent_config_data from_json_file(const std::string &file_path);

  /// load config options to enable_trackers
  void load_config_options_to_trackers();

  /// agent_http_server_port
