...
func main() {
	...
	for _, s := range config.Ipv4BlacklistPerm {
		ipu32, erri := ipToUint32(s)
		if erri != nil {
			fmt.Println(erri)
			continue
	...
	*/
	...
	if isXDPDetached(config.InterfaceName) {
		fmt.Println("卸载了")
	} else {
		fmt.Println("挂载中")
	}
...
func isXDPDetached(interfaceName string) bool {
	// 检查接口上的 XDP 状态
	xdpPath := fmt.Sprintf("/sys/class/net/%s/xdp", interfaceName)
	content, err := os.ReadFile(xdpPath)
	if err != nil {
		log.Fatalf("Error reading %s: %v", xdpPath, err)
		return false
	}

	// 如果文件内容是 "none"，说明程序已从接口分离
	return strings.TrimSpace(string(content)) == "none"
}
